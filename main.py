    ...
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .platform-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; vertical-align: middle; }
            .wr-select {
                border: 1px solid var(--border-color); border-radius: var(--radius-sm); background: var(--card-bg);
                padding: 10px 12px; font-family: inherit; font-size: 13px; font-weight: 700; color: var(--text-dark);
                box-shadow: var(--shadow-card); cursor: pointer;
            }
            .wr-section { background: var(--card-bg); border-radius: var(--radius-lg); box-shadow: var(--shadow-card); padding: 22px; margin-bottom: 18px; }
            .wr-section h2 { font-size: 16px; font-weight: 800; margin: 0 0 4px; color: var(--text-dark); }
            .wr-section .wr-sub { font-size: 12px; color: var(--text-muted); margin-bottom: 16px; font-weight: 500; }
            .wr-kpis { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 18px; }
            .wr-kpi { border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 14px 16px; background: var(--nested-bg); }
            .wr-kpi-label { font-size: 12px; color: var(--text-muted); font-weight: 700; margin-bottom: 6px; }
            .wr-kpi-value { font-family: 'Manrope', 'Cairo', sans-serif; font-size: 24px; font-weight: 800; color: #0b1b33; direction: ltr; unicode-bidi: isolate; display: inline-block; }
            .wr-kpi-unit { font-size: 12px; color: var(--text-muted); font-weight: 700; margin-right: 4px; }
            .wr-kpi .compare-pill { margin-top: 8px; }
            .wr-table-wrap { overflow-x: auto; }
            .wr-table { width: 100%; border-collapse: collapse; font-size: 13px; min-width: 620px; }
            .wr-table th { background: var(--nested-deep); color: var(--text-muted); font-size: 12px; font-weight: 700; text-align: right; padding: 10px 12px; position: static; cursor: default; }
            .wr-table td { padding: 10px 12px; border-bottom: 1px solid var(--border-color); white-space: nowrap; }
            .wr-table tr.wr-best td { background: var(--stage-green-bg); }
            .wr-table td b { font-weight: 800; }
            .wr-notes { display: grid; gap: 10px; }
            .wr-note { display: flex; gap: 12px; align-items: flex-start; padding: 12px 14px; border-radius: var(--radius-sm); font-size: 13.5px; line-height: 1.8; font-weight: 600; }
            .wr-note .wr-note-title { font-weight: 800; white-space: nowrap; }
            .wr-note.gain { background: var(--stage-green-bg); color: var(--stage-green-text); }
            .wr-note.issue { background: var(--stage-red-bg); color: var(--stage-red-text); }
            .wr-note.reco { background: var(--stage-blue-bg); color: var(--stage-blue-text); }
            .wr-note span.wr-note-body { color: var(--text-dark); }
            .wr-platform { border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 16px; margin-bottom: 14px; }
            .wr-platform-head { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 12px; flex-wrap: wrap; }
            .wr-platform-name { font-size: 15px; font-weight: 800; display: flex; align-items: center; gap: 8px; }
            .wr-action { font-size: 12px; font-weight: 800; padding: 5px 12px; border-radius: 999px; }
            .wr-action.scale { background: var(--stage-green-bg); color: var(--stage-green-text); }
            .wr-action.maintain { background: var(--stage-blue-bg); color: var(--stage-blue-text); }
            .wr-action.creative { background: var(--accent-orange-light); color: var(--accent-orange); }
            .wr-action.budget { background: var(--stage-red-bg); color: var(--stage-red-text); }
            .wr-meta-line { font-size: 12.5px; color: var(--text-muted); font-weight: 600; margin-top: 10px; line-height: 1.8; }
            .wr-meta-line b { color: var(--text-dark); }
            .wr-decisions { margin: 0; padding-right: 20px; display: grid; gap: 10px; font-size: 14px; line-height: 1.8; font-weight: 600; }
            .wr-footnote { font-size: 11.5px; color: var(--text-muted); margin-top: 10px; line-height: 1.7; }
            .wr-kpi { min-width: 0; }
            .wr-kpi .compare-pill { white-space: normal; }
            @media (max-width: 1100px) { .wr-kpis { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
            @media (max-width: 860px) {
                .wr-section { padding: 16px; }
                .wr-kpi { padding: 12px; }
                .wr-kpi-label { font-size: 11px; }
                .wr-kpi-value { font-size: 18px; }
                .wr-select { grid-column: 1 / -1; width: 100%; min-width: 0; }
            }
    ...
    <body>
        ...
        <div class="sidebar" id="sidebar">
            ...
            <ul class="sidebar-menu">
                <li class="active" id="nav-dashboard"><a href="#" onclick="showView('dashboard', event)">📊 الإعلانات والتحليلات</a></li>
                ...
                <li id="nav-weekly"><a href="#" onclick="showView('weekly', event)">📈 تقارير الأداء الأسبوعية</a></li>
            </ul>
        ...
        <div class="main-content">
          ...
          <div id="view-weekly" style="display:none;">
            <div class="top-bar">
                <div class="header-title">
                    <h1>تقرير الأداء الأسبوعي للإعلانات الممولة</h1>
                    <p id="weekly-subtitle">تقرير تنفيذي للإدارة - يُقدَّم كل خميس ويغطي 7 أيام (الجمعة ← الخميس)</p>
                </div>
                <div class="top-actions">
                    <select id="weekly-week-select" class="wr-select" onchange="loadWeeklyReport(Number(this.value))"></select>
                    <button class="btn btn-copy" onclick="copyWeeklyReport()">📋 نسخ التقرير</button>
                    <button class="btn btn-copy" onclick="copyWeeklyReportForAI()">🤖 نسخ للتحليل</button>
                    <button class="btn btn-refresh" onclick="loadWeeklyReport(weeklyState.offset, true)">تحديث</button>
                </div>
            </div>
            <div id="weekly-body"><div class="empty-state">جاري إعداد التقرير...</div></div>
          </div>
        </div>
...
_تم إعداد هذا التقرير آلياً عبر منصة Elevenz_`;
            ...
            // ═══════════════════════════════════════════════════════════════
            // تقرير الأداء الأسبوعي للإدارة: يُقدَّم كل خميس ويغطي 7 أيام من الجمعة
            // إلى الخميس، مع مقارنة بأسبوع الخميس السابق. كل الأرقام والتوصيات محسوبة
            // بقواعد واضحة من البيانات الفعلية (لا تخمين).
            // ═══════════════════════════════════════════════════════════════
            const WR_NL = String.fromCharCode(10);
            const WR_SEP = '━━━━━━━━━━━━━━━━';
            // حملات هدفها الزيارات/الوصول/المشاهدات لا تُنتج عملاء بطبيعتها - تُستبعد من
            // حساب "تكلفة العميل الفعلية" وقرارات الميزانية حتى لا تظلم المنصة التي تشغّلها.
            const WR_AWARENESS_RE = /TRAFFIC|REACH|VIDEO_VIEW|AWARENESS/i;
            const WR_PLATFORMS = [
                { key: 'meta', label: 'Meta (فيسبوك/إنستغرام)', short: 'Meta', dataKey: 'meta_ads', dot: '#0866ff', leadsWord: 'محادثة/عميل' },
                { key: 'tiktok', label: 'TikTok', short: 'TikTok', dataKey: 'tiktok_ads', dot: '#fe2c55', leadsWord: 'تحويل' },
                { key: 'google', label: 'Google Ads', short: 'Google', dataKey: 'google_ads', dot: '#fbbc04', leadsWord: 'تحويل' }
            ];
            let weeklyState = { offset: 0, data: null };

            function wrWeekRange(offset) {
                // آخر خميس حتى اليوم (إن كان اليوم خميساً فهو نهاية الأسبوع الحالي)
                const end = new Date();
                end.setHours(0, 0, 0, 0);
                end.setDate(end.getDate() - ((end.getDay() - 4 + 7) % 7) - 7 * offset);
                const start = new Date(end);
                start.setDate(start.getDate() - 6);
                return { from: toIsoDate(start), to: toIsoDate(end) };
            }

            function wrLeadsOf(i, key) {
                if (key === 'meta') return parseMetaConversions(i);
                if (key === 'google') return parseGoogleConversions(i);
                return safeNum(i.conversions || i.conversion || i.results);
            }
            function wrIsAwareness(i) {
                return WR_AWARENESS_RE.test(String(i.campaign_objective || i.objective_type || ''));
            }

            function wrPlatformMetrics(list, key) {
                const m = { spend: 0, impressions: 0, clicks: 0, leads: 0, awarenessSpend: 0 };
                scopedList(list).forEach(i => {
                    const spend = safeNum(i.spend || i.cost);
                    m.spend += spend;
                    m.impressions += safeNum(i.impressions);
                    m.clicks += safeNum(i.clicks);
                    m.leads += wrLeadsOf(i, key);
                    if (wrIsAwareness(i)) m.awarenessSpend += spend;
                });
                m.ctr = m.impressions > 0 ? m.clicks / m.impressions * 100 : 0;
                m.cpc = m.clicks > 0 ? m.spend / m.clicks : null;
                m.cpl = m.leads > 0 ? m.spend / m.leads : null;
                const leadSpend = m.spend - m.awarenessSpend;
                m.leadCpl = m.leads > 0 ? leadSpend / m.leads : null;
                m.active = m.spend > 0;
                return m;
            }

            // الإعلان الرابح: أقل تكلفة عميل بين الإعلانات التي حققت 3 عملاء فأكثر
            // (أو أكثر الإعلانات عملاءً إن لم يبلغ أي إعلان 3)، من حملات العملاء فقط.
            function wrWinningAd(list, key) {
                const SEP = String.fromCharCode(1);
                const map = {};
                scopedList(list).filter(i => !wrIsAwareness(i)).forEach(i => {
                    const k = campaignOf(i) + SEP + adOf(i);
                    if (!map[k]) map[k] = { campaign: campaignOf(i), ad: adOf(i), spend: 0, leads: 0, clicks: 0, impressions: 0 };
                    map[k].spend += safeNum(i.spend || i.cost);
                    map[k].leads += wrLeadsOf(i, key);
                    map[k].clicks += safeNum(i.clicks);
                    map[k].impressions += safeNum(i.impressions);
                });
                const ads = Object.values(map).filter(a => a.leads > 0);
                ads.forEach(a => { a.cpl = a.spend / a.leads; a.ctr = a.impressions > 0 ? a.clicks / a.impressions * 100 : 0; });
                const solid = ads.filter(a => a.leads >= 3).sort((a, b) => a.cpl - b.cpl);
                if (solid.length) return solid[0];
                return ads.sort((a, b) => b.leads - a.leads)[0] || null;
            }

            function wrWastedAds(list, key) {
                const SEP = String.fromCharCode(1);
                const map = {};
                scopedList(list).filter(i => !wrIsAwareness(i)).forEach(i => {
                    const k = campaignOf(i) + SEP + adOf(i);
                    if (!map[k]) map[k] = { ad: adOf(i), spend: 0, leads: 0 };
                    map[k].spend += safeNum(i.spend || i.cost);
                    map[k].leads += wrLeadsOf(i, key);
                });
                return Object.values(map).filter(a => a.leads === 0 && a.spend >= 50);
            }

            function wrPct(cur, prev) {
                if (cur === null || prev === null || cur === undefined || prev === undefined || prev === 0) return null;
                return (cur - prev) / prev * 100;
            }
            function wrPill(cur, prev, goodDirection, label) {
                const html = buildComparePillHtml(cur, prev, { goodDirection, label: label || 'مقابل الأسبوع السابق' });
                return html || '<span class="compare-pill flat">لا توجد مقارنة</span>';
            }
            function wrChangeText(cur, prev, lowerIsBetter) {
                const c = wrPct(cur, prev);
                if (c === null) return '';
                if (Math.abs(c) < 0.5) return ' (مستقر)';
                const arrow = c > 0 ? '▲' : '▼';
                const good = lowerIsBetter ? c < 0 : c > 0;
                return ` (${arrow}${Math.abs(c).toFixed(1)}%${lowerIsBetter === undefined ? '' : good ? ' ✅' : ' ⚠️'})`;
            }
            const wrMoney = v => v === null || v === undefined ? '—' : fmt(v);

            // الإجراء الفوري لكل منصة - قواعد صريحة على تكلفة العميل الفعلية والنقر
            function wrPlatformAction(cur, prev) {
                const leadSpend = cur.spend - cur.awarenessSpend;
                if (leadSpend >= 50 && cur.leads === 0) return { cls: 'budget', label: 'إعادة توزيع الميزانية', why: 'صرف على حملات العملاء بدون أي عميل هذا الأسبوع' };
                const cplChange = wrPct(cur.leadCpl, prev.leadCpl);
                const leadsChange = wrPct(cur.leads, prev.leads);
                const ctrChange = wrPct(cur.ctr, prev.ctr);
                if (cplChange !== null && cplChange <= -10 && (leadsChange === null || leadsChange >= 0))
                    return { cls: 'scale', label: 'توسيع (Scale)', why: `تكلفة العميل انخفضت ${Math.abs(cplChange).toFixed(0)}% مع ثبات أو نمو العملاء` };
                if (cplChange !== null && cplChange >= 20 && ctrChange !== null && ctrChange <= -15)
                    return { cls: 'creative', label: 'تعديل الإعلانات (Creative)', why: `تكلفة العميل ارتفعت ${cplChange.toFixed(0)}% مع انخفاض النقر ${Math.abs(ctrChange).toFixed(0)}% - مؤشر إرهاق الإعلانات` };
                if (cplChange !== null && cplChange >= 20)
                    return { cls: 'budget', label: 'إعادة توزيع الميزانية', why: `تكلفة العميل ارتفعت ${cplChange.toFixed(0)}% دون تراجع في النقر - المنافسة/الجمهور هو السبب المرجّح` };
                return { cls: 'maintain', label: 'تثبيت (Maintain)', why: 'الأداء مستقر ضمن المعدل المعتاد' };
            }

            function wrBuildInsights(rows, total, prevTotal) {
                const leadRows = rows.filter(r => r.cur.active && r.cur.leadCpl !== null);
                const best = leadRows.slice().sort((a, b) => a.cur.leadCpl - b.cur.leadCpl)[0] || null;
                const worst = leadRows.length > 1 ? leadRows.slice().sort((a, b) => b.cur.leadCpl - a.cur.leadCpl)[0] : null;

                let gain;
                const leadsChange = wrPct(total.leads, prevTotal.leads);
                const cplChange = wrPct(total.cpl, prevTotal.cpl);
                if (best) {
                    gain = `${best.p.short} الأعلى كفاءة بتكلفة ${fmt(best.cur.leadCpl)} ر.س للعميل (${best.cur.leads.toLocaleString('en-US')} عميل)`;
                    if (leadsChange !== null && leadsChange > 0) gain += `، وإجمالي العملاء ارتفع ${leadsChange.toFixed(0)}% عن الأسبوع السابق`;
                    else if (cplChange !== null && cplChange < 0) gain += `، والتكلفة الشاملة للعميل انخفضت ${Math.abs(cplChange).toFixed(0)}%`;
                    gain += '.';
                } else {
                    gain = 'لم تُسجَّل عملاء كافية هذا الأسبوع لتحديد مكسب واضح.';
                }

                let issue = null;
                const zero = rows.find(r => r.cur.spend - r.cur.awarenessSpend >= 50 && r.cur.leads === 0);
                const rising = rows.filter(r => wrPct(r.cur.leadCpl, r.prev.leadCpl) >= 20).sort((a, b) => wrPct(b.cur.leadCpl, b.prev.leadCpl) - wrPct(a.cur.leadCpl, a.prev.leadCpl))[0];
                if (zero) issue = `${zero.p.short} صرفت ${fmt(zero.cur.spend - zero.cur.awarenessSpend)} ر.س على حملات العملاء بدون أي عميل.`;
                else if (rising) issue = `ارتفاع تكلفة العميل في ${rising.p.short} بنسبة ${wrPct(rising.cur.leadCpl, rising.prev.leadCpl).toFixed(0)}% (من ${fmt(rising.prev.leadCpl)} إلى ${fmt(rising.cur.leadCpl)} ر.س).`;
                else if (cplChange !== null && cplChange >= 10) issue = `ارتفاع التكلفة الشاملة للعميل ${cplChange.toFixed(0)}% مقارنة بالأسبوع السابق.`;
                else issue = 'لا توجد مشكلة جوهرية هذا الأسبوع - الأداء مستقر.';

                let reco;
                if (best && worst && best !== worst && best.cur.leadCpl < worst.cur.leadCpl * 0.8) {
                    reco = `نقل 15-20% من ميزانية ${worst.p.short} (تكلفة العميل ${fmt(worst.cur.leadCpl)} ر.س) إلى ${best.p.short} (${fmt(best.cur.leadCpl)} ر.س) بدءاً من الجمعة.`;
                } else if (best) {
                    reco = `الإبقاء على توزيع الميزانية الحالي، مع زيادة تدريجية 10% لـ ${best.p.short} إذا ثبتت تكلفة العميل خلال أول 3 أيام.`;
                } else {
                    reco = 'مراجعة أهداف الحملات وإعدادات التتبع قبل ضخ ميزانية إضافية.';
                }
                return { best, worst, gain, issue, reco };
            }

            function wrBuildDecisions(rows, insights) {
                const d = [insights.reco];
                const winner = rows.filter(r => r.winner).sort((a, b) => a.winner.cpl - b.winner.cpl)[0];
                if (winner) d.push(`إنتاج نسختين جديدتين بنفس زاوية الإعلان الرابح "${winner.winner.ad}" على ${winner.p.short} (تكلفة العميل ${fmt(winner.winner.cpl)} ر.س) واختبارهما مقابل الأصلي.`);
                const wasted = rows.flatMap(r => r.wasted.map(w => Object.assign({ platform: r.p.short }, w)));
                if (wasted.length) {
                    const sum = wasted.reduce((s, w) => s + w.spend, 0);
                    d.push(`إيقاف أو استبدال ${wasted.length} إعلان صرفت ${fmt(sum)} ر.س هذا الأسبوع بدون أي عميل (${wasted.slice(0, 3).map(w => w.ad + ' - ' + w.platform).join('، ')}${wasted.length > 3 ? '...' : ''}).`);
                } else {
                    const creative = rows.find(r => r.action.cls === 'creative');
                    if (creative) d.push(`تحديث إعلانات ${creative.p.short} لوجود مؤشر إرهاق (انخفاض النقر وارتفاع التكلفة).`);
                    else d.push('إضافة إعلانين جديدين للاختبار على المنصة الأقل كفاءة لرفع جودة العملاء وخفض التكلفة.');
                }
                return d.slice(0, 3);
            }

            async function wrFetchRange(r) {
                const res = await fetch(`/api/data?date_from=${r.from}&date_to=${r.to}`);
                const json = await res.json();
                if (json.status !== 'success') throw new Error(json.message || 'تعذر جلب البيانات');
                return json.data || {};
            }
            ...
            function renderWeekSelect() {
                ...
                for (let k = 0; k < 8; k++) {
                    const r = wrWeekRange(k);
                    const opt = document.createElement('option');
                    opt.value = k;
                    opt.textContent = `${k === 0 ? 'أسبوع الخميس الأخير' : 'أسبوع الخميس'} ${formatDisplayDate(r.to)}`;
                    sel.appendChild(opt);
                }
            }

            async function loadWeeklyReport(offset, force) {
                renderWeekSelect();
                offset = offset || 0;
                const sel = document.getElementById('weekly-week-select');
                if (sel) sel.value = offset;
                if (!force && weeklyState.data && weeklyState.offset === offset) return renderWeeklyReport();
                const body = document.getElementById('weekly-body');
                body.innerHTML = '<div class="empty-state">جاري إعداد التقرير...</div>';
                const range = wrWeekRange(offset), prevRange = wrWeekRange(offset + 1);
                try {
                    const [cur, prev] = await Promise.all([wrFetchRange(range), wrFetchRange(prevRange)]);
                    weeklyState = { offset, range, prevRange, cur, prev, data: true };
                    renderWeeklyReport();
                } catch (e) {
                    body.innerHTML = `<div class="empty-state">${escapeHtml(e.message || 'تعذر إعداد التقرير')}</div>`;
                }
            }

            function wrCompute() {
                const { cur, prev } = weeklyState;
                const rows = WR_PLATFORMS.map(p => {
                    const c = wrPlatformMetrics(cur[p.dataKey], p.key);
                    const pr = wrPlatformMetrics(prev[p.dataKey], p.key);
                    return { p, cur: c, prev: pr, action: wrPlatformAction(c, pr), winner: wrWinningAd(cur[p.dataKey], p.key), wasted: wrWastedAds(cur[p.dataKey], p.key) };
                }).filter(r => r.cur.active || r.prev.active);
                const sum = key => rows.reduce((s, r) => s + r.cur[key], 0);
                const psum = key => rows.reduce((s, r) => s + r.prev[key], 0);
                const total = { spend: sum('spend'), leads: sum('leads') };
                total.cpl = total.leads > 0 ? total.spend / total.leads : null;
                const prevTotal = { spend: psum('spend'), leads: psum('leads') };
                prevTotal.cpl = prevTotal.leads > 0 ? prevTotal.spend / prevTotal.leads : null;
                const insights = wrBuildInsights(rows, total, prevTotal);
                const decisions = wrBuildDecisions(rows, insights);
                return { rows, total, prevTotal, insights, decisions };
            }

            function renderWeeklyReport() {
                const body = document.getElementById('weekly-body');
                const { range, prevRange } = weeklyState;
                const r = wrCompute();
                document.getElementById('weekly-subtitle').innerText =
                    `تقرير أسبوع الخميس ${formatDisplayDate(range.to)} | الفترة: الجمعة ${formatDisplayDate(range.from)} ← الخميس ${formatDisplayDate(range.to)} | المقارنة مع: ${formatDisplayDate(prevRange.from)} ← ${formatDisplayDate(prevRange.to)}`;

                if (!r.rows.length) {
                    body.innerHTML = '<div class="empty-state">لا توجد بيانات إعلانية لهذا الأسبوع</div>';
                    return;
                }
                const kpi = (label, value, unit, pill) => `
                    <div class="wr-kpi">
                        <div class="wr-kpi-label">${label}</div>
                        <div><span class="wr-kpi-value">${value}</span><span class="wr-kpi-unit">${unit}</span></div>
                        ${pill}
                    </div>`;
                const leadsChange = wrPct(r.total.leads, r.prevTotal.leads);

                const compRows = r.rows.map(x => {
                    const share = r.total.spend > 0 ? x.cur.spend / r.total.spend * 100 : 0;
                    const isBest = r.insights.best && r.insights.best.p.key === x.p.key;
                    return `<tr class="${isBest ? 'wr-best' : ''}">
                        <td><b><span class="platform-dot" style="background:${x.p.dot}"></span> ${x.p.short}</b>${isBest ? ' 🏆' : ''}</td>
                        <td>${fmt(x.cur.spend)} ر.س</td>
                        <td>${x.cur.leads.toLocaleString('en-US')}</td>
                        <td>${wrMoney(x.cur.cpl)}${x.cur.awarenessSpend > 0 && x.cur.leadCpl !== null ? ` <span style="color:var(--text-muted);font-size:11px;">(فعلية ${fmt(x.cur.leadCpl)})</span>` : ''}</td>
                        <td>${share.toFixed(1)}%</td>
                    </tr>`;
                }).join('');

                const platformBlocks = r.rows.map(x => {
                    const c = x.cur, pv = x.prev;
                    const line = (label, cv, pvv, fmtFn, dir) => `<tr><td>${label}</td><td><b>${fmtFn(cv)}</b></td><td>${fmtFn(pvv)}</td><td>${wrPill(cv, pvv, dir, ' ')}</td></tr>`;
                    const money = v => v === null ? '—' : fmt(v) + ' ر.س';
                    const int = v => Number(v || 0).toLocaleString('en-US');
                    const pct = v => Number(v || 0).toFixed(2) + '%';
                    const w = x.winner;
                    return `
                    <div class="wr-platform">
                        <div class="wr-platform-head">
                            <div class="wr-platform-name"><span class="platform-dot" style="background:${x.p.dot}"></span>${x.p.label}</div>
                            <span class="wr-action ${x.action.cls}" title="${escapeHtml(x.action.why)}">الإجراء: ${x.action.label}</span>
                        </div>
                        <div class="wr-table-wrap"><table class="wr-table">
                            <thead><tr><th>المؤشر</th><th>هذا الأسبوع</th><th>الأسبوع السابق</th><th>التغير</th></tr></thead>
                            <tbody>
                                ${line('الصرف', c.spend, pv.spend, money, 'neutral')}
                                ${line('الظهور', c.impressions, pv.impressions, int, 'up')}
                                ${line('النقرات', c.clicks, pv.clicks, int, 'up')}
                                ${line('CTR', c.ctr, pv.ctr, pct, 'up')}
                                ${line('تكلفة النقرة CPC', c.cpc, pv.cpc, money, 'down')}
                                ${line('العملاء المحتملين (Leads)', c.leads, pv.leads, int, 'up')}
                                ${line('تكلفة العميل CPL', c.cpl, pv.cpl, money, 'down')}
                            </tbody>
                        </table></div>
                        <div class="wr-meta-line">
                            <b>سبب الإجراء:</b> ${escapeHtml(x.action.why)}<br>
                            <b>الإعلان الرابح:</b> ${w ? `${escapeHtml(w.ad)} <span style="color:var(--text-faint)">(حملة: ${escapeHtml(w.campaign)})</span> - ${w.leads.toLocaleString('en-US')} ${x.p.leadsWord} بتكلفة ${fmt(w.cpl)} ر.س للعميل` : 'لا يوجد إعلان حقق عملاء هذا الأسبوع'}
                            ${c.awarenessSpend > 0 ? `<br><b>ملاحظة:</b> منها ${fmt(c.awarenessSpend)} ر.س على حملات زيارات/وصول (هدفها ليس العملاء)، وتكلفة العميل الفعلية لحملات العملاء: ${wrMoney(c.leadCpl)} ر.س` : ''}
                        </div>
                    </div>`;
                }).join('');
                ...
                body.innerHTML = `
                    <div class="wr-section">
                        <h2>1. الملخص التنفيذي الموحد</h2>
                        <div class="wr-sub">كافة القنوات المربوطة - مقارنة بتقرير الخميس ${formatDisplayDate(prevRange.to)}</div>
                        <div class="wr-kpis">
                            ${kpi('إجمالي الإنفاق الإعلاني', fmt(r.total.spend), 'ر.س', wrPill(r.total.spend, r.prevTotal.spend, 'neutral'))}
                            ${kpi('إجمالي العملاء والاستفسارات', r.total.leads.toLocaleString('en-US'), 'عميل', wrPill(r.total.leads, r.prevTotal.leads, 'up'))}
                            ${kpi('متوسط تكلفة العميل الشاملة (Blended CPL)', wrMoney(r.total.cpl), 'ر.س', wrPill(r.total.cpl, r.prevTotal.cpl, 'down'))}
                            ${kpi('تغير عدد العملاء', leadsChange === null ? '—' : (leadsChange >= 0 ? '+' : '') + leadsChange.toFixed(1) + '%', '', '<span class="compare-pill flat">مقابل الأسبوع السابق</span>')}
                        </div>
                        <div class="wr-table-wrap"><table class="wr-table">
                            <thead><tr><th>المنصة</th><th>الميزانية المنفقة</th><th>العملاء المحتملين</th><th>تكلفة العميل CPL</th><th>حصة الميزانية</th></tr></thead>
                            <tbody>${compRows}</tbody>
                        </table></div>
                        <div class="wr-footnote">Snapchat غير مربوطة بالمنصة حالياً، فلا تظهر في التقرير. العملاء في Meta = محادثات الرسائل التي بدأها العملاء.</div>
                    </div>

                    <div class="wr-section">
                        <h2>الملاحظة التنفيذية</h2>
                        <div class="wr-sub">3 نقاط مباشرة للإدارة</div>
                        <div class="wr-notes">
                            <div class="wr-note gain"><span class="wr-note-title">✅ المكسب:</span><span class="wr-note-body">${escapeHtml(r.insights.gain)}</span></div>
                            <div class="wr-note issue"><span class="wr-note-title">⚠️ التحدي:</span><span class="wr-note-body">${escapeHtml(r.insights.issue)}</span></div>
                            <div class="wr-note reco"><span class="wr-note-title">💡 التوصية:</span><span class="wr-note-body">${escapeHtml(r.insights.reco)}</span></div>
                        </div>
                    </div>

                    <div class="wr-section">
                        <h2>2. أداء كل منصة على حدة</h2>
                        <div class="wr-sub">الأرقام الأساسية مع المقارنة بالأسبوع السابق، والإعلان الرابح، والإجراء المقترح</div>
                        ${platformBlocks}
                    </div>

                    <div class="wr-section">
                        <h2>3. خطة وتوصيات الأسبوع القادم</h2>
                        <div class="wr-sub">قرارات تبدأ من يوم الجمعة ${formatDisplayDate((() => { const d = new Date(range.to + 'T00:00:00'); d.setDate(d.getDate() + 1); return toIsoDate(d); })())}</div>
                        <ol class="wr-decisions">${r.decisions.map(d => `<li>${escapeHtml(d)}</li>`).join('')}</ol>
                    </div>`;
            }
            ...
            function buildWeeklyReportText() {
                ...
                const r = wrCompute();
                const L = [];
                L.push(`*تقرير الأداء الأسبوعي للإعلانات الممولة*`);
                L.push(`تقرير أسبوع الخميس ${formatDisplayDate(range.to)}`);
                L.push(`الفترة: ${formatDisplayDate(range.from)} ← ${formatDisplayDate(range.to)}`);
                L.push(WR_SEP);
                L.push('*1. الملخص التنفيذي*');
                L.push(`• إجمالي الإنفاق: ${fmt(r.total.spend)} ر.س${wrChangeText(r.total.spend, r.prevTotal.spend)}`);
                L.push(`• إجمالي العملاء والاستفسارات: ${r.total.leads.toLocaleString('en-US')}${wrChangeText(r.total.leads, r.prevTotal.leads, false)}`);
                L.push(`• متوسط تكلفة العميل الشاملة: ${wrMoney(r.total.cpl)} ر.س${wrChangeText(r.total.cpl, r.prevTotal.cpl, true)}`);
                L.push('');
                L.push('*مقارنة المنصات*');
                r.rows.forEach(x => {
                    const share = r.total.spend > 0 ? x.cur.spend / r.total.spend * 100 : 0;
                    L.push(`• ${x.p.short}: ${fmt(x.cur.spend)} ر.س | ${x.cur.leads.toLocaleString('en-US')} عميل | CPL ${wrMoney(x.cur.cpl)} | ${share.toFixed(0)}% من الميزانية`);
                });
                L.push('');
                L.push('*الملاحظة التنفيذية*');
                L.push(`✅ المكسب: ${r.insights.gain}`);
                L.push(`⚠️ التحدي: ${r.insights.issue}`);
                L.push(`💡 التوصية: ${r.insights.reco}`);
                L.push(WR_SEP);
                L.push('*2. أداء المنصات*');
                r.rows.forEach(x => {
                    const c = x.cur, pv = x.prev;
                    L.push('');
                    L.push(`*${x.p.label}*`);
                    L.push(`الصرف ${fmt(c.spend)} ر.س${wrChangeText(c.spend, pv.spend)} | الظهور ${c.impressions.toLocaleString('en-US')} | النقرات ${c.clicks.toLocaleString('en-US')}`);
                    L.push(`CTR ${c.ctr.toFixed(2)}%${wrChangeText(c.ctr, pv.ctr, false)} | CPC ${wrMoney(c.cpc)} ر.س | العملاء ${c.leads.toLocaleString('en-US')}${wrChangeText(c.leads, pv.leads, false)} | CPL ${wrMoney(c.cpl)} ر.س${wrChangeText(c.cpl, pv.cpl, true)}`);
                    if (x.winner) L.push(`🏆 الإعلان الرابح: ${x.winner.ad} - ${x.winner.leads} عميل بتكلفة ${fmt(x.winner.cpl)} ر.س`);
                    L.push(`➡️ الإجراء: ${x.action.label} - ${x.action.why}`);
                });
                L.push(WR_SEP);
                L.push('*3. قرارات الأسبوع القادم*');
                r.decisions.forEach((d, i) => L.push(`${i + 1}. ${d}`));
                L.push('');
                L.push('_Elevenz_');
                return L.join(WR_NL);
            }
            ...
            function copyWeeklyReport() {
                if (!weeklyState.data) return;
                doCopyText(buildWeeklyReportText(), 'تم نسخ التقرير الأسبوعي 📋');
            }

            function copyWeeklyReportForAI() {
                if (!weeklyState.data) return;
                const { range } = weeklyState;
                const text = [
                    'أنت مستشار تسويق رقمي وخبير في إعداد التقارير التنفيذية للإدارة العليا (C-Level).',
                    'أريدك أن تصيغ لي "تقرير الأداء الأسبوعي للإعلانات الممولة" لشركتنا في قطاع الديكور والتأثيث.',
                    `ملاحظة دورة التقرير: الإعلانات تعمل طوال أيام الأسبوع (7 أيام)، والتقرير يُقدَّم كل خميس ويغطي الفترة من الجمعة ${formatDisplayDate(range.from)} حتى الخميس ${formatDisplayDate(range.to)}.`,
                    '',
                    'المطلوب إخراج التقرير بصيغة تنفيذية، مبسطة، وموجهة للنتائج المالية، وفق الهيكل التالي:',
                    `1. الملخص التنفيذي الموحد (تقرير أسبوع الخميس ${formatDisplayDate(range.to)}): بطاقة مؤشرات (إجمالي الإنفاق، إجمالي العملاء، Blended CPL، نسبة التغير مقارنة بالخميس الماضي) + جدول مقارنة المنصات (المنصة | الميزانية | العملاء | CPL | حصة الميزانية %) + 3 نقاط تنفيذية (المكسب، التحدي، توصية توزيع الميزانية).`,
                    '2. تقرير أداء كل منصة على حدة: الأرقام الأساسية مع المقارنة بالأسبوع السابق، الإعلان الرابح وزاوية المحتوى، والإجراء الفوري (Scale / Maintain / Creative / إعادة توزيع ميزانية).',
                    '3. خطة الأسبوع القادم: 3 قرارات واضحة لتحسين جودة وتكلفة العملاء بدءاً من الجمعة.',
                    '',
                    'البيانات الفعلية للأسبوع (مستخرجة تلقائياً من منصة Elevenz، مع المقارنة بالأسبوع السابق):',
                    '',
                    buildWeeklyReportText()
                ].join(WR_NL);
                doCopyText(text, 'تم نسخ التقرير للتحليل - الصقه في Claude 🤖');
            }

            // التنقل بين صفحة اللوحة وصفحة التقرير الأسبوعي
            function showView(view, evt) {
                ...
                if (view === 'weekly') loadWeeklyReport(weeklyState.offset || 0);
            }
